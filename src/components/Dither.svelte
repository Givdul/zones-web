...
void main() { vUv = uv; gl_Position = vec4(position, 1.0); }`;
  ...
  onMount(() => {
    ...
    const mouse = new THREE.Vector2(containerRef.clientWidth / 2, containerRef.clientHeight / 2);
    ...
    let raf = 0;
    const render = () => {
      if (!current.disableAnimation && !reducedMotion.matches) waveUniforms.time.value = clock.getElapsedTime();
...
<div bind:this={containerRef} class="dither"></div>
